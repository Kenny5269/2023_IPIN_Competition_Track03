...
% Reservar memoria
Posi=ones(numlineas,4)*NaN; index_Posi=1;
Acce=ones(numlineas,5)*NaN; index_Acce=1;
Gyro=ones(numlineas,5)*NaN; index_Gyro=1;
Magn=ones(numlineas,5)*NaN; index_Magn=1;
...
Ahrs=ones(numlineas,8)*NaN; index_Ahrs=1;
...
while (~eof)
    ...
    while (~linea_con_medida && ~eof)
        ...
        if ( idx_linea<=numlineas) % hay lineas por procesar
            ...
            % GNSS/GPS data:        'GNSS;AppTimestamp(s);SensorTimeStamp(s);Latit(Âº);Long(Âº);Altitude(m);Bearing(Âº);Accuracy(m);Speed(m/s);SatInView;SatInUse'
            ...
            if (numel(linea)>0 && ~strcmp(linea(1),'%') )
                ...
                if ( strfind(linea,'GNSS'))  % Es una linea de GPS
                    % 'GNSS;Timestamp(s);SensorTimeStamp(s);Latitude(Âº);Longitude(Âº);Altitude(m);Bearing(Âº),Accuracy(m),Speed(m/s),satellites_in_view;num_satellites_in_use'
                ...
                if ( strfind(linea,'WIFI'))  % Es una linea de WIFI.
                    ...
                    cell_datos{1}=cell_array{1,1}; % timestamp
                    %cell_datos{2}=cell_array{1,2}; % Sensortimestamp
                    cell_datos{3}=cell_array{1,4}; % Frequency
					cell_datos{4}=cell_array{1,5}; % RSS
                    %MAC_str=cell_array{1,3}{1,1}; % MAC string
                    ...
                    cell_datos{2}=cell_array{1,3}{1,1}; % MAC cell array
                    tipo='WIFI';
                    % disp(['Linea: ',linea]);
                end
              ...
              % NEW BLE 4.0 data:       	'BLE4;AppTimestamp(s);iBeacon;MAC;RSSI(dBm);Power;MajorID;MinorID;UUID'
                    ...
                    cell_datos{1}=cell_array{1,1}; % timestamp
                    %cell_datos{4}=cell_array{1,4}; % Major
                    %cell_datos{5}=cell_array{1,5}; % Minor
                    UUID=strrep(cell2mat(cell_array{1,6}),'-',''); % UUID
                    cell_datos{3}=cell_array{1,3}; % RSSI
                    %MAC_str=cell_array{1,2}{1,1}; % MAC
                    %mac_clean = strrep(MAC_str, ':', '');
                    %mac_number = str2double(mac_clean);
                    %MAC_dec_array=sscanf(MAC_str,'%x:%x:%x:%x:%x:%x');
                    %MAC_dec=MAC_dec_array(1)*256^5+MAC_dec_array(2)*256^4+MAC_dec_array(3)*256^3+MAC_dec_array(4)*256^2+MAC_dec_array(5)*256+MAC_dec_array(6);
                    cell_datos{2}=cell_array{1,2}{1,1}; % MAC
                    tipo='BLE4';
                    % disp(['Linea: ',linea]);
                end
                if ( strfind(linea,'IMUL'))  % Es una linea de IMUL. p.ej.: "IMUL;0.929;1907.2437;11782;0.22992;-0.15328;9.31184;-0.01853;0.01550;0.00004;-24.02985;56.41791;22.50000;-1.28843;1.03222;121.61204;936.870;0.00"
                    % 21 campos: Timestamp(s);SensorTimestamp(s);Counter;Acc_X(m/s^2);Acc_Y(m/s^2);Acc_Z(m/s^2);Gyr_X(rad/s);Gyr_Y(rad/s);Gyr_Z(rad/s);Mag_X(uT);;Mag_Y(uT);Mag_Z(uT);Roll(ï¿?);Pitch(ï¿?);Yaw(ï¿?);Quat1;Quat2;Quat3;Quat4;Pressure(mbar);Temp(ï¿½C)'");
            ...
            if ~isempty(datos) || ~isempty(cell_datos)
    ...
    if strcmp(tipo,'POSI')
        Posi(index_Posi,1:4)=datos;   index_Posi=index_Posi+1;
    end
    if strcmp(tipo,'ACCE')
        Acce(index_Acce,1:5)=datos;   index_Acce=index_Acce+1;
    end
    if strcmp(tipo,'GYRO')
        Gyro(index_Gyro,1:5)=datos;   index_Gyro=index_Gyro+1;
    end
    if strcmp(tipo,'MAGN')
        Magn(index_Magn,1:5)=datos;   index_Magn=index_Magn+1;
    end
    if strcmp(tipo,'PRES')
    ...
    if strcmp(tipo,'AHRS')
        Ahrs(index_Ahrs,1:8)=datos;   index_Ahrs=index_Ahrs+1;
    end
    if strcmp(tipo,'GNSS')
    ...
    if strcmp(tipo,'WIFI')
        Wifi(index_Wifi, :) = cell_datos;   index_Wifi=index_Wifi+1;
    end
    if strcmp(tipo,'IMUL')
...
%     figure(idx_fig); idx_fig=idx_fig+1;
%     subplot(4,2,1);
%     plot(Gnss(:,1),Gnss(:,3),'r-');ylabel('Lat(ï¿?)');
%     title(['GNSS data - Freq: ',num2str(size(Gnss,1)/Gnss(end,1)),' Hz']);
%     subplot(4,2,2);
%     plot(Gnss(:,1),Gnss(:,4),'r-'); ylabel('Long(ï¿?)');
%     title(['Resolution=[1e-4ï¿?, 1.1 m, 0.3 m/s] ']);
%     subplot(4,2,3);
%     plot(Gnss(:,1),Gnss(:,5),'r-'); ylabel('Height(m)');
%     subplot(4,2,4);
%     plot(Gnss(:,1),Gnss(:,7),'r-'); ylabel('Accuracy(m)');
%     subplot(4,2,5);
%     plot(Gnss(:,1),Gnss(:,6),'r-'); ylabel('Bearing(ï¿?)');
%     subplot(4,2,6);
...
%     figure(idx_fig); idx_fig=idx_fig+1;
%     % 21 campos: Timestamp(s);LPMSTimestamp(s);Counter;Acc_X(m/s^2);Acc_Y(m/s^2);Acc_Z(m/s^2);Gyr_X(rad/s);Gyr_Y(rad/s);Gyr_Z(rad/s);Mag_X(uT);;Mag_Y(uT);Mag_Z(uT);Roll(ï¿?);Pitch(ï¿?);Yaw(ï¿?);Pressure(mbar);Temp(ï¿½C)'");
...
%     xlabel('time(s)'); ylabel('Orientation (ï¿?)');
...
%     subplot(2,1,2);
%     plot(tiempos,Imul(:,21),'r-'); ylabel('Temperature (ï¿½C)');
%     xlabel('time(s)');
% 
% end
% 
...
%     figure(idx_fig); idx_fig=idx_fig+1;
%     % 21 campos: Timestamp(s);SensorTimestamp(s);Counter;Acc_X(m/s^2);Acc_Y(m/s^2);Acc_Z(m/s^2);Gyr_X(rad/s);Gyr_Y(rad/s);Gyr_Z(rad/s);Mag_X(uT);;Mag_Y(uT);Mag_Z(uT);Roll(ï¿?);Pitch(ï¿?);Yaw(ï¿?);Pressure(mbar);Temp(ï¿½C)'");
%     % indices de Imul: Acc en(4:6), Gyr en(7:9), Mag en(10:12), Euler en(13:15), Quat(16:19),
%     %                  Pres en 20 y Temp en 21
...
%     xlabel('time(s)'); ylabel('Orientation (Âº)');
...
%     subplot(2,1,2);
%     plot(tiempos,Imux(:,21),'r-'); ylabel('Temperature (ÂºC)');
%     xlabel('time(s)');
% 
% end
