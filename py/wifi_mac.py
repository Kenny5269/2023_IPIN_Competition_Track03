...
if __name__ == '__main__':
    ...
    # 步驟6：整理欄位名稱集合
    rssi_columns = list(pivot_rss.columns)
    freq_columns = list(pivot_freq.columns)

    column_info_df = pd.DataFrame({
        'wifi_rssi_columns': rssi_columns + [''] * (len(freq_columns) - len(rssi_columns)) if len(freq_columns) > len(rssi_columns) else rssi_columns,
        'wifi_freq_columns': freq_columns + [''] * (len(rssi_columns) - len(freq_columns)) if len(rssi_columns) > len(freq_columns) else freq_columns
    })

    # 步驟7：輸出欄位名稱集合為 CSV
    column_info_df.to_csv('WIFI_column_names.csv', index=False)

    print("整理完成！輸出檔案為 WIFI_merged.csv 和 WIFI_column_names.csv")
