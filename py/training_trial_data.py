...
def plot_figure(aligned_data_train, aligned_data_test):
    ...
    gt_coords_origin = [(d["gt_lat"], d["gt_lon"]) for d in aligned_data_train]
    gt_coords = [(d["gt_lat"], d["gt_lon"]) for d in aligned_data_test]
    init_coords = [(d["init_lat"], d["init_lon"]) for d in aligned_data_test]
    ...
    fused_coords = [(d["fused_lat"], d["fused_lon"]) for d in aligned_data_test]
    # print(len(gt_coords))
...
def evaluate_errors(aligned_data, pred_key_lat="fused_lat", pred_key_lon="fused_lon", plot_cdf=True):
    ...
    for d in aligned_data:
        if d["gt_lat"] is not None and d["gt_lon"] is not None:
            gt_coords.append([d["gt_lat"], d["gt_lon"]])
            pred_coords.append([d[pred_key_lat], d[pred_key_lon]])

    gt_coords = np.array(gt_coords)
    pred_coords = np.array(pred_coords)

    dists = np.array([
        geodesic(gt, pred).meters
