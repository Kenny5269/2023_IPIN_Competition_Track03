...
index2 = [24,25,26,27]
# input_file = 'T53_R2'
# INPUT_WIFI_CSV = f'py/{input_file}/WIFI_merged_filtered_top6.csv'
...
def estimate_step_length_from_world_acc(df, start, end, window=20, method='instant'):
    """
...
def estimate_trajectory_from_imu_all(aligned_data, this_idx, end_idx, imu_df, heading):
    ...
    for idx in peaks:
        ...
        print(f'last idx = {last_idx}, this idx = {idx}, step length = {step_length}')
        #print(step_length)
...
def estimate_trajectory_from_imu_all_old(aligned_data, this_idx, end_idx, imu_df, heading):
    ...
    for idx in peaks:
        ...
        # step_length = estimate_step_length_from_world_acc(imu_df, last_idx, idx, method='integrate') / 6
